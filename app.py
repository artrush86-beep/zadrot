...
def ask_ai(user_message: str, user_name: str = "", context: str = "",
    ...
    """Единая точка вызова AI: перебор провайдеров по порядку fallback."""
    resp = multi_ai.ask(user_message, user_name=user_name, context=context, history=history)
    if resp and user_id:
        try:
            if not _redis_save_hist(user_id, user_message, resp):
                save_chat_message(user_id, user_message, resp)
        except Exception:
            pass
    return resp
...
def ai_available() -> bool:
    """Есть ли хотя бы один настроенный AI-провайдер."""
    return multi_ai.available
...
def cmd_ai(m):
    ...
    if response:
        label = f"🤖 {multi_ai.last_provider}"
...
def handle_message(message):
    ...
    if msg_count % 10 == 0 and ai_available():
    ...
    if should_random_reply() and ai_available():
        uid_r = user.id
...
def _job_morning():
    ...
    if ai_available():
        ai_greeting = multi_ai.generate_greeting("чат", is_morning=True)
        if ai_greeting:
            text = ai_greeting
